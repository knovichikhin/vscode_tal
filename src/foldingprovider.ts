...
export class TALFoldingProvider implements vscode.FoldingRangeProvider {
  ...
  public async provideFoldingRanges(
    ...
    const foldableCommentBlock: vscode.FoldingRange = new vscode.FoldingRange(
      -2,
      -2,
   ...
   */
  private parseToggles(
    line: string,
    lineNum: number,
    foldableToggles: FoldableToggle[]
    ...
    switch (toggle[1]) {
      ...
      case "if":
        foldableToggles.push(<FoldableToggle>{
          foldable: new vscode.FoldingRange(
            lineNum,
            lineNum,
      ...
      case "endif":
        ...
        index = this.lastToggleIndex(foldableToggles, toggle[2], [
          ToggleType.if,
          ToggleType.ifnot,
        ]);
        if (index > -1) {
   ...
   */
  private lastToggleIndex(
    foldables: FoldableToggle[],
    toggle_id: string,
    toggle_types: ToggleType[]
  ): number {
  ...
  private parseCommentBlock(
    line: string,
    lineNum: number,
    foldableCommentBlock: vscode.FoldingRange
  ): vscode.FoldingRange | undefined {
    let result: vscode.FoldingRange | undefined = undefined;
   ...
   * @return Either a FoldingRange if found or undefined otherwise.
   */
  private parseBeginEnd(
    line: string,
    lineNum: number,
    foldableStacks: vscode.FoldingRange[]
  ): vscode.FoldingRange | undefined {
    let result: vscode.FoldingRange | undefined = undefined;
